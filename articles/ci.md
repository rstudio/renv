...
In some CI workflows, you may want to generate or update a lockfile
without installing packages – for example, to validate that dependencies
can be resolved, or to produce a lockfile artifact in a lightweight
container that doesn’t have compilation tools.

[`renv::plan()`](https://rstudio.github.io/renv/reference/plan.md)
resolves the packages required by a project and writes a lockfile,
...
Because
[`renv::plan()`](https://rstudio.github.io/renv/reference/plan.md) works
from repository metadata rather than installed packages, it can run in
environments where package installation would be impractical.

## Downloading packages in CI

In multi-stage CI pipelines, it can be useful to download packages in
one step and install them in a later step.
...
[`renv::restore()`](https://rstudio.github.io/renv/reference/restore.md)
can install from the cache without re-downloading.
