...
from an Posit Connect content bundle into an `renv.lock` lockfile.

This function can be useful when you need to recreate the package
environment of a piece of content that is deployed to Posit Connect. The
content bundle contains a `manifest.json` file that is used to recreate
the package environment. This function will let you convert that
manifest file to an `renv.lock` file. Run
