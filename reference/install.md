...
[`restore()`](https://rstudio.github.io/renv/reference/restore.md) (i.e.
it uses cached packages where possible) but it does not respect the
...
- repos:
  ...
  [config](https://rstudio.github.io/renv/reference/config.md) for an
  alternate way override.
...
- transactional:
  ...
  **Transactional Restore** in
  [`restore()`](https://rstudio.github.io/renv/reference/restore.md) for
  more details. When `NULL` (the default), the value of the
  `install.transactional`
  [`config`](https://rstudio.github.io/renv/reference/config.md) option
  will be used.
