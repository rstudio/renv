...
#' Infer if a parsed remote is a git repo
#'
#' For a given url, eg:
...
#' Hints include:
#'
#' - `git::` prefix- Though this is ususally parsed into `type`
#' - `git@` login of `git`
#'   - Though you can usually use your actual username, most `ssh`
#' git urls (eg, those that github generates) use a `git@` prefix.
#' - A path extension of `.git` for the repo
#'   - This is in autogenerated github urls
#' - A `ssh://` protocol
#'   - This is one way to specify the ssh protocol is used for transport.
#'   - It's not included in github generated urls, but it is in the git spec.
renv_remotes_is_git <- function(parsed) {
  return(
    "git" %in% c(
      parsed$type,
      parsed$login,
