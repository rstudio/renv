...
This ensures that once you turn on renv for a project, it stays on, until you deliberately turn it off.
...
This is useful because you can then share the lockfile and other people or other computers can easily reproduce your current environment by running `renv::restore()`, which reads that metadata from the lockfile.
...
You'll then need to commit `renv.lock`, `.Rprofile`, `renv/settings.json` and `renv/activate.R` to version control.
...
For development and collaboration, the `renv/library` directory should be ignored and all others checked in to version control.
