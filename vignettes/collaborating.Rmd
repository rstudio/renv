...
they will be able to use `renv::init()` to initialize your project with exactly
the same R packages as recorded in the lockfile. This implies the following
workflow for collaboration:

1. Select a way to share your project sources. The recommended way to do so is
...
also execute `renv::init()` to initialize their project with the packages
...
packages that you were when `renv.lock` was generated.
...
your collaborators, and request that they execute `renv::init()` to
synchronize their library with the lockfile. Or, if the project has already
been initialized on their machine via a prior call to `renv::init()`, then they
...
`renv.lock` are communicated so that everyone knows and understands when and
