...
[RStudio Connect](https://rstudio.com/products/connect/) is a publication
...
with the `writeManifest()` function from the `rsconnect` package. When using
`renv`, the only thing that needs to be considered is that `rsconnect` should be
...
project dependencies from the local `renv` library.
