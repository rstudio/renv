...
# restore the R project library
RUN R -e 'renv::restore()'
```

Note that we create a user library (from `R_LIBS_USER`) to avoid potential
issues with read-only or write-only mounts in the Docker image.
