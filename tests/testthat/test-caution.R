...
test_that("caution() creates bulleted list with optional postamble", {
  expect_snapshot({
...
test_that("caution() doesn't show pre/post amble if no values", {
  expect_silent(caution("before", character(), "after"))
})
...
test_that("options(renv.pretty.print.emitter) is respected", {
  ...
  expect_condition(caution("preamble", 1), class = cls)
